...
import time
    ...
    # problem1 의 설명
set_number=random.randint(2,5)                                                           # differ problem2
...
for i in range(set_number):
    turn=1
    while(turn != 0):
        xcoor = random.randint(2, 50)                                                       # differ problem2
        if(xcoor==0):
            break
        else : listx.append(xcoor)
        ycoor = random.randint(2, 50)
        if (ycoor == 0):
            listx.remove(-1)
            break
...
colorlist=['blue', 'purple', 'red', 'green', 'black', 'silver']
for i in range(set_number):
    print("Here is input of set%s " %(i+1), coorlist[i])
